...
SP1_HyPB_M_0.25kbp_narrow <- read.table("SP1_HyPB/ENCODE/Mitrapeaks/0.25kb_Windowoutput_MitraSP1-HyPB_CCpeaks_p9_SP1_ChIP_fq_ENCODE.bed", header = TRUE, sep = "\t", col.names = col_names)
HyPB_0.25kbp_M_narrow <- read.table("SP1_HyPB/ENCODE/Mitrapeaks/undirected/0.25kb_Windowoutput_MitraHyPB_CCpeaks_p9_SP1_ChIP_fq_ENCODE.bed", header = TRUE, sep = "\t", col.names = col_names)
...
SP1_HyPB_M_TotalPeaks <- nrow(SP1_HyPB_M_0.25kbp_narrow)
HyPB_M_TotalPeaks <- nrow(HyPB_0.25kbp_M_narrow)
TTAAtotal <- nrow(GenoTTAA)
#Then we count the number of reads with no overlap - this is denoted by a '0' in the overlap column

SP1_HyPB_M_0.25kbp_a_peaks_b_overlap <- as.numeric(length(which(SP1_HyPB_M_0.25kbp_narrow$Number.of.Overlaps.with.ChIP.Seq.Peaks==0)))
HyPB_M_0.25kbp_a_peaks_b_overlap <- as.numeric(length(which(HyPB_0.25kbp_M_narrow$Number.of.Overlaps.with.ChIP.Seq.Peaks==0)))
TTAA_0.25kbp_a_peaks_b_overlap <- as.numeric(length(which(GenoTTAA$Number.of.Overlaps.with.ChIP.Seq.Peaks==0)))

#Then we can work out percentage of overlapped reads from the number of non-overlapped reads
Percent_SP1_HyPB_0.25kbp_M <- (1-(SP1_HyPB_M_0.25kbp_a_peaks_b_overlap/SP1_HyPB_M_TotalPeaks))*100
Percent_HyPB_0.25kbp_M <- (1-(HyPB_M_0.25kbp_a_peaks_b_overlap/HyPB_M_TotalPeaks))*100
Percent_TTAA_0.25kbp <- (1-(TTAA_0.25kbp_a_peaks_b_overlap/TTAAtotal))*100
...
Final_Percentage_table <- data.frame(Name=Order,
                                     ...
                                     Overlap_withChIP= c(SP1_HyPB_M_0.25kbp_a_peaks_b_overlap, HyPB_M_0.25kbp_a_peaks_b_overlap, TTAA_0.25kbp_a_peaks_b_overlap),
                                     PercentageOverlapping= c(Percent_SP1_HyPB_0.25kbp_M, Percent_HyPB_0.25kbp_M, Percent_TTAA_0.25kbp))
