...
#setwd ("C:/Users/JackP/Documents/GitHub/Local Calling Cards Analysis Datastore/Data/Moudgil/Moudgil_2020/20230925WindowOutputs")
...
SP1_HyPB_MM_1kbp_narrow <- read.table("SP1_HyPB/Mitra/Mitrapeaks/1kb_Windowoutput_MitraSP1-HyPB_CCpeaks_p9_SP1_ChIP_fq_Mitra.bed", header = TRUE, sep = "\t", col.names = col_names)
HyPB_1kbp_MM_narrow <- read.table("SP1_HyPB/Mitra/Mitrapeaks/undirected/1kb_Windowoutput_MitraHyPB_CCpeaks_p9_SP1_ChIP_fq_Mitra.bed", header = TRUE, sep = "\t", col.names = col_names)
                 ...
                 aes(x= factor(Name, Order), 
                     ...
                     fill = Name)) +  geom_col(color = "black",
                                               show.legend = FALSE) +  ylim(0, 100) + labs(x = "Name and Distance from ChIP-Seq Peak", 
                                                                                           ...
                                                                                           subtitle = "Determining percentage of SP1-HyPB and HyPB peaks from Mitra \npeak files within 1000 bp of SP1 ChIP-Seq peaks using Mitra control") +  theme_dark() + coord_cartesian(expand = FALSE, clip = "off") + geom_hline(yintercept = 83, linetype = "dashed", linewidth = 1) 
