...
ESR1_H5_HyPB_A90 = read.table("20231017outputs/1kbp_A_ER_HyPB_HF552DSX7_L2_1_fdr_bh_0.05_no-r_peaks.bed_B_GSM2970404_A90_peaks.bed", header = TRUE, sep = "\t", col.names = col_names)
Undir_H5_HyPB_A90 = read.table("20231017outputs/1kbp_A_WT_HyPB_HF552DSX7_L2_1_p30_peaks.bed_B_GSM2970404_A90_peaks.bed", header = TRUE, sep = "\t", col.names = col_names)
...
ESR1_total=nrow(ESR1_H5_HyPB_A90)
HyPB_total=nrow(Undir_H5_HyPB_A90)
TTAA_total=nrow(TTAA_A90)

#Count the number of reads with no overlap
ESR1_overlap = as.numeric(length(which(ESR1_H5_HyPB_A90$Number.of.Overlaps.with.ChIP.Seq.Peaks==0)))
HyPB_overlap = as.numeric(length(which(Undir_H5_HyPB_A90$Number.of.Overlaps.with.ChIP.Seq.Peaks==0)))
...
A90_H5_Finalpercentage_table = data.frame(Name=c('ESR1-HyPB 1kbp', 'Undirected HyPB 1kbp', 'Genomic TTAA 1kbp'),
...
A90_H5_Figure <- ggplot(data = A90_H5_Finalpercentage_table, 
