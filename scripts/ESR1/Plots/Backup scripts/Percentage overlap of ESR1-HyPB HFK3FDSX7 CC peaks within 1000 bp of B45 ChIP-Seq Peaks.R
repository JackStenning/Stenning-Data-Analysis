...
ESR1_HyPB_B45 = read.table("20231017outputs/1kbp_A_ER_HyPB_HFK3FDSX7_L2_1_fdr_bh_0.05_no-r_peaks.bed_B_GSM2970406_B45_peaks.bed", header = TRUE, sep = "\t", col.names = col_names)
Undir_HyPB_B45 = read.table("20231017outputs/1kbp_A_WT_HyPB_HFK3FDSX7_L2_1_p30_peaks.bed_B_GSM2970406_B45_peaks.bed", header = TRUE, sep = "\t", col.names = col_names)
TTAA_B45 = read.table("20231017outputs/1kbp_A_hg38_TTAA.bed_B_GSM2970406_B45_peaks.bed", header = TRUE, sep = "\t", col.names = Genocol_names)

#Counting the total reads
ESR1_total=nrow(ESR1_HyPB_B45)
HyPB_total=nrow(Undir_HyPB_B45)
TTAA_total=nrow(TTAA_B45)

#Count the number of reads with no overlap
ESR1_overlap = as.numeric(length(which(ESR1_HyPB_B45$Number.of.Overlaps.with.ChIP.Seq.Peaks==0)))
HyPB_overlap = as.numeric(length(which(Undir_HyPB_B45$Number.of.Overlaps.with.ChIP.Seq.Peaks==0)))
TTAA_overlap = as.numeric(length(which(TTAA_B45$Number.of.Overlaps.with.ChIP.Seq.Peaks==0)))
...
B45_Finalpercentage_table = data.frame(Name=c('ESR1-HyPB 1kbp', 'Undirected HyPB 1kbp', 'Genomic TTAA 1kbp'),
...
B45_Figure <- ggplot(data = B45_Finalpercentage_table, 
